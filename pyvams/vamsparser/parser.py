...
# - derived from pyverilog/vparser/parser.py by Shinya Takamaeda-Yamazaki
# License: Apache 2.0
#-------------------------------------------------------------------------------
...
from pyvams.vamsparser.ply import yacc
from pyvams.vamsparser.plyparser import PLYParser, Coord, ParseError
from pyvams.vamsparser.preprocessor import VerilogAMSPreprocessor
from pyvams.vamsparser.lexer import VerilogAMSLexer
from pyvams.vamsparser.ast import *

class VerilogAMSParser(PLYParser):
    """VerilogAMS HDL Parser"""
    ...
    def __init__(self):
        self.lexer = VerilogAMSLexer(error_func=self._lexer_error_func)
        self.lexer.build()
    ...
    def p_expression_unot(self, p):
        'expression : SYM_NOT expression %prec UNOT'
        p[0] = Unot(p[2])

    def p_expression_uand(self, p):
        'expression : SYM_AND expression %prec UAND'
        p[0] = Uand(p[2])

    def p_expression_unand(self, p):
        'expression : SYM_NAND expression %prec UNAND'
        p[0] = Unand(p[2])

    def p_expression_unor(self, p):
        'expression : SYM_NOR expression %prec UNOR'
        p[0] = Unor(p[2])

    def p_expression_uor(self, p):
        'expression : SYM_OR expression %prec UOR'
        p[0] = Uor(p[2])

    def p_expression_uxor(self, p):
        'expression : SYM_XOR expression %prec UXOR'
        p[0] = Uxor(p[2])

    def p_expression_uxnor(self, p):
        'expression : SYM_XNOR expression %prec UXNOR'
        p[0] = Uxnor(p[2])
    ...
    def p_expression_And(self, p):
        'expression : expression SYM_AND expression'
        p[0] = And(p[1], p[3])

    def p_expression_Xor(self, p):
        'expression : expression SYM_XOR expression'
        p[0] = Xor(p[1], p[3])

    def p_expression_Xnor(self, p):
        'expression : expression SYM_XNOR expression'
        p[0] = Xnor(p[1], p[3])

    ######################################################################
    # Level 9
    def p_expression_Or(self, p):
        'expression : expression SYM_OR expression'
        p[0] = Or(p[1], p[3])
    ...
    def p_levelsigs(self, p):
        'levelsigs : levelsigs OR levelsig'
        p[0] = p[1] + (p[3],)
    ...
    def p_systemcall_noargs(self, p):
        'systemcall : DOLLAR ID'
        p[0] = SystemCall(p[2], ())

    def p_systemcall(self, p):
        'systemcall : DOLLAR ID LPAREN sysargs RPAREN'
        p[0] = SystemCall(p[2], p[4])

    def p_systemcall_signed(self, p):#for $signed system task
        'systemcall : DOLLAR SIGNED LPAREN sysargs RPAREN'
        p[0] = SystemCall(p[2], p[4])
...
#-------------------------------------------------------------------------------
class VerilogAMSCodeParser(object):
    def __init__(self, filelist, preprocess_output='preprocess.output',
        ...
        self.directives = ()
        self.preprocessor = VerilogAMSPreprocessor(filelist, preprocess_output,
                                                preprocess_include,
                                                preprocess_define)
        self.parser = VerilogAMSParser()
...
def parse(filelist, preprocess_include=None, preprocess_define=None):
    codeparser = VerilogAMSCodeParser(filelist,
...
if __name__ == '__main__':
    ...
    INFO = "VerilogAMS code parser with Pyvams"
